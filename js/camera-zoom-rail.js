...
function cameraActive() {
  return Boolean(cameraToggleButton?.classList.contains('active'));
}

function zoomAvailable() {
  return Boolean(zoomInput && !zoomInput.disabled && cameraActive());
}
...
function makeButton(kind, symbol, label) {
...
function install() {
  if (!rail || rail.querySelector('[data-camera-zoom]')) {
    syncButtons();
    return;
  }
...
if (cameraToggleButton) {
  const observer = new MutationObserver(() => setTimeout(syncButtons, 220));
  observer.observe(cameraToggleButton, { attributes: true, attributeFilter: ['class'] });
}

// Kamera açıldıktan sonra yeteneklerin okunması biraz sürebilir.
